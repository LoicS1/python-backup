...
def hangman_display(guess, secret):
    ...
    parameter). If the character matches with the a character in the secret word it will
...
def hangman():
    ...
    and then calls the hangman_display function as long enough lives remain.  
    After each execution of the hangman diplay function, this function checks:
        if the user has cheated by guessing more than one character at a time, in which 
    ...
    time.sleep(1)
    print ("You will be prompted to guess the name of a famous person. Please " +
    "guess only one lowercase letter at a time.")
    time.sleep(2)
    lives = 7
    secret = random.choice(celeb_list) #selects a Celebrity name from the list
    print ("secret: " + secret)  #used only for testing of game
    time.sleep(1)
    print ("You have " + str(lives) + " lives in this game. You will not lose a" + 
    " life if you guess a letter in the secret word.")
    # changed after to gallery walk to explain to users in what conditions they will lose a life
